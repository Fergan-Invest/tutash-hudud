...
@section('breadcrumb', $requestItem->request_number)

@section('content')
<section class="detail-hero">
    <div>
        <p class="eyebrow">Ariza</p>
...
<div class="detail-actions">
    @can('update', $requestItem)<a class="secondary-button" href="{{ route('requests.edit', $requestItem) }}">Tahrirlash</a>@endcan
</div>
...
<div class="detail-grid">
    <section class="panel">
        ...
        <div class="detail-list">
            ...
            <div><span>Maydon</span><strong>{{ $requestItem->total_area }} m²</strong></div>
            <div><span>Faoliyat</span><strong>{{ $requestItem->activity_type }}</strong></div>
        </div>
    </section>
    <aside class="panel">
        <div class="panel-heading"><h2>Online va yaratuvchi</h2></div>
...
<section class="panel">
    ...
    <div class="document-grid">
        @foreach($requestItem->images as $image)<a href="{{ Storage::url($image->path) }}" target="_blank">{{ $image->original_name }}</a>@endforeach
        @foreach($requestItem->files as $file)<a href="{{ Storage::url($file->path) }}" target="_blank">{{ $file->type }}: {{ $file->original_name }}</a>@endforeach
    </div>
</section>
